...
let Temp = []
const minIndexSeam = []
let countIndexSeam = 0;
console.log('****************************************************************')
while (count < width * height) {
  minIndexSeam[countIndexSeam] = getArrayChunk(M_matrix)
  // count = count + height
  count += height
  countIndexSeam++
  console.log(minIndexSeam)
}

function getArrayChunk(M_matrix) {
  let min = M_matrix[count];
  let index = count;
  for (let i = count; i < count + height; i++) {
    // console.log(min,M_matrix[i])
    if (min > M_matrix[i]) {
      min = M_matrix[i];
      index = i;
    }
